        ...
        for(auto &q: queries){
            ...
            if(low<high){
                ...
                if(high-low==1){
                    ...
                    int pairOne=firstLen+blockSize[low+1];
                    int pairLast=blockSize[high-1]+lastLen;
                    int RMQMaxPairSum=RMQ(st,N,low+1,high-2); //It was high-2
                    maxPairSum=max(RMQMaxPairSum,max(pairOne,pairLast));
                }
            }
